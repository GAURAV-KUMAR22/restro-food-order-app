...
export const DashBoard = () => {
  ...
  useEffect(() => {
    ...
    const fetchedVisitor = async () => {
      ...
      setVisitorData(response.data.content);
    };
  ...
  return (
    ...
    "content": {
        ...
        <div className="bg-gray-800 p-4 rounded-lg shadow">
          ...
          <p className="text-3xl font-bold">{visitorData.totalVisits}</p>
          <p className="text-3xl font-bold">{visitorData.totalUniqueVisitors}</p>
          <p className="text-sm text-green-400">+9.5% compared to last week</p>
          <ul className="mt-2 space-y-1 text-sm text-gray-300">
            <li>📱 Mobile: <span>{visitorData.devices.mobile?visitorData.devices.mobile:0}</span></li>
            <li>💻 Desktop: <span>{visitorData.devices.desktop?visitorData.devices.desktop:0}</span></li>
            <li>📟 Tablet: <span>{visitorData.devices.tablet?visitorData.devices.tablet:0}</span></li>
          </ul>
        </div>
      </div>
