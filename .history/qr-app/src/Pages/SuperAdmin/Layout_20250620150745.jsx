...
export const SuperAdminLayout = () => {
  ...
  return (
    <div className="p-4 md:p-6 bg-white rounded-lg shadow-md min-h-screen relative">
      {/* Header */}
      <div className="w-full flex justify-between items-center mb-6 px-2 font-semibold sm:flex-col">
        ...
        <div className="flex space-x-4 h-10 items-center">
          ...
          />
          <div className="relative" onClick={() => setOpenComplaintModel(true)}>
            <BellIcon size={25} />
            {complaints.length > 0 && (
                ...
                <ul className="flex flex-col space-y-1">
                  ...
                  <li>
                    ...
                    >
                      Complaints
                      {complaints.length > 0 && (
                        <span className="absolute top-2 right-3 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white text-xs font-semibold">
                          {complaints.length}
                        </span>
                      )}
                    </button>
      ...
      {openComplaintModel && (
        <div className="fixed inset-0 z-50 flex justify-center items-center drop-shadow-lg bg-opacity-30">
          <div className="relative w-[700px] h-[700px] bg-white shadow-2xl rounded-lg overflow-hidden">
            <img
              src={cross}
              alt="cross"
              className="absolute right-0 top-2 text-red-600 w-[35px] h-[35px]"
              onClick={() => setOpenComplaintModel(false)}
            />
            <h1 className="text-2xl text-gray-700 font-semibold text-center py-4 border-b">
              Complaints
            </h1>
            <div className="px-6 py-4 space-y-4 overflow-y-auto h-auto">
              {complaints.map((item, index) => (
                ...
                >
                  ...
                  <p className="font-medium">
                    Admin ID:{" "}
                    <span className="font-normal">{item.adminId._id}</span>
                  </p>
                  <p className="font-medium">
                    Admin Name:{" "}
                    <span className="font-normal">{item.adminId.name}</span>
                  </p>
                  <p className="font-medium">
                    complainDetail :{" "}
                    <span className="font-normal">{item.complainDetail}</span>
                  </p>
                  <p className="font-medium">
                  ...
                  <details>
                    <Link
                      to={`${backendUrl}/${item.snapshot}`}
                      className="w-full h-auto"
                    >
                      <img src={`${backendUrl}/${item.snapshot}`} />
                    </Link>
                  </details>
                </div>
              ))}
