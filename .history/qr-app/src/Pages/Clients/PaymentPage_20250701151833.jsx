...
export const PaymentPage = () => {
  ...
  const handleCashOrder = async () => {
    setLoading(true);
    try {
      const response = await publicAxios.post("/orders/place-order", {
        userId,
        items: mappedItems,
  ...
  const handleUPIFlow = async () => {
    setLoading(true);
    try {
      const res = await publicAxios.post("/api/payment/create-order-client", {
        userId,
        amount: totalPrice,
        paymentMethod: "UPI",
        shopId,
        ...
        handler: async (response) => {
          try {
            const verifyRes = await publicAxios.post(
              "/api/payment/verify-payment-client",
              {
            ...
            if (verifyRes.status === 200) {
              await publicAxios.post("/orders/place-order", {
                userId,
                items: mappedItems,
                totalAmount: totalPrice,
                paymentMethod: "UPI",
      ...
      toast.error("Error initiating UPI payment");
    } finally {
      setLoading(false);
    }
  };
