...
export const Login = () => {
  ...
  async function handleForm(e) {
    ...
    if (validate()) {
      try {
        const response = await PrivateAxios.post("/auth/login", form);
        ...
        dispatch(
          loginSuccess({
            ...
            isSubscribed: response.data.content.subscribe, // or calculate it here
          })
        );
        ...
        if (content.subscribe) {
          naviagate("/admin/dashboard"); // or service route
        } else {
          naviagate("/admin/package"); // plan page
        }
      } catch (err) {
