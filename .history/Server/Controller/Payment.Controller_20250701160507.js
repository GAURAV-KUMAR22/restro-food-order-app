...
import crypto from "crypto";
import razorpay from "../config/razorpay"; // Ensure Razorpay is initialized
...
export const verifyPayment_Order = async (req, res) => {
  try {
    ...
    const missingFields = {};
    if (!razorpay_order_id) missingFields.razorpay_order_id = "Missing";
    if (!razorpay_payment_id) missingFields.razorpay_payment_id = "Missing";
    if (!razorpay_signature) missingFields.razorpay_signature = "Missing";
    if (!userId) missingFields.userId = "Missing";
    if (!orderId) missingFields.orderId = "Missing";
    if (!paymentMethod) missingFields.paymentMethod = "Missing";
    if (!amount) missingFields.amount = "Missing";
