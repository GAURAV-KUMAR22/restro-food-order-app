...
export const LoginUser = async (req, res) => {
  ...
  try {
    ...
    const isPaidAdmin =
      new Date(isExistingUser.subscription.expiresAt) > new Date();

    console.log("sjdjbhsbhdsj d", isPaidAdmin);
    const data = {
      isApproved: isExistingUser.isApproved,
      subscribe: !!isPaidAdmin,
      user: {
        _id: isExistingUser._id,
        name: isExistingUser.name,
        coverImage: isExistingUser.coverImage,
      },
      subscription: isExistingUser.subscription,
