...
export const postCheckoutPayment = async (req, res) => {
  const { packageTitle, price, email, adminId, packageId, startDate, endDate } =
    req.body;
  ...
  customer_creation: "always", // creates a Stripe customer

  // ✅ Collect required details
  shipping_address_collection: {
    allowed_countries: ["IN"], // restrict or allow based on where you sell
  },
  billing_address_collection: "required", // ✅ Force collection of billing address

  // ✅ Pass full name and optional extra details
  metadata: {
    customer_name: "Gaurav Kumar", // get from user input
    adminId,
    packageId,
    startDate,
    endDate,
    ...
    res
      .status(500)
      .json({ message: "Checkout session failed", error: error.message });
  }
};
