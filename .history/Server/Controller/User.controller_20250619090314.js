...
export const getVisitorInfo = async (req, res) => {
  try {
    const visitorInfo = VisitorInfo.find({});
    const uniqueIPs = await visitorInfo.distinct("ip");
    if (!visitorInfo) {
      return res.status(400).json({ content: null });
    }

    const laptop = visitorInfo.screen.width >= 1536;
    const mobile = visitorInfo.screen.width < 400
    const data = {
      totalVisitors: uniqueIPs.length,
      mobile:visitorInfo.
    };

    res.status(200).json({ content: visitorInfo });
  } catch (error) {
