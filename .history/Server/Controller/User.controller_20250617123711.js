...
export const getAllComplain = async (req, res) => {
  ...
  try {
    const allComplaint = await Complain.find({});
...
export const patchComplain = async (req, res) => {
  ...
  const existingComplain = await Complain.findOne({ _id });
  if (!existingComplain) {
    return res.status(404);
  }
...
export const getCoverImage = async (req, res) => {
  ...
  try {
    const existingAdmin = await Admin.findOne({ _id }).select("-password");
    if (!existingAdmin) {
      return res.status(404);
    }
    if (existingAdmin.coverImage.length === 0) {
      return res.status(200).json({ content: null });
    }
    const image = { coverImage: existingAdmin.coverImage };
